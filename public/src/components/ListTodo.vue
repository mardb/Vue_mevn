...
<div>
  <div>
    <h3> Todo Items</h3>
     <div class="row mrb-10" > <!--v-for="todo in todos" not working-->
      ...
      <span class="help-block small" v-show="todo.editing">Press enter to update</span>
    </div>
  </div>
...
export default {
  ...
  methods: {
    fetchTodo(){
      let uri = 'http://localhost:5500/api/all';
    ...
    updateTodo(todo){
      let id= todo._id;
      let uri = 'http://localhost:5500/api/update' + id;
      todo.editing = false;
    ...
    deleteTodo(id) {
      let uri = 'http://localhost:5500/api/delete' + id;
      axios.get(uri);
      this.fetchTodo();
    },
    listenToEvents(){
      bus.$on('refreshTodo', ($event) => {
        this.fetchTodo(); //refresh/update list
      })
    }
  }
}
